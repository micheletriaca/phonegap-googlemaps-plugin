...
public class PluginMarker extends MyPlugin {
	private ConcurrentHashMap<String, Bitmap> cache = null;
	
	private void createMarker_(final JSONObject opts, final CountDownLatch taskCountdown, final ConcurrentHashMap<Integer, JSONObject> results, final Integer taskIdx) throws JSONException {
		if (cache == null) {
	...
	private void createMarkers(final JSONArray args, final CallbackContext callbackContext) throws JSONException {
		final JSONArray markersArgs = args.getJSONArray(1);
		ConcurrentHashMap<Integer, JSONObject> results = new ConcurrentHashMap<Integer, JSONObject>(markersArgs.length());
		CountDownLatch taskCountDown = new CountDownLatch(markersArgs.length());
		for (Integer i = 0; i < markersArgs.length(); i++){
			this.createMarker_(markersArgs.getJSONObject(i), taskCountDown, results, i);
		}
		try{
			taskCountDown.await();
		...
		for (Integer i = 0; i < markersArgs.length(); i++){
			resultsArray.put(i, results.get(i));
		}
		callbackContext.success(resultsArray);
	}
	...
	private void createMarker(final JSONArray args, final CallbackContext callbackContext) throws JSONException {
		ConcurrentHashMap<Integer, JSONObject> results = new ConcurrentHashMap<Integer, JSONObject>(1);
		CountDownLatch taskCountDown = new CountDownLatch(1);
		this.createMarker_(args.getJSONObject(1), taskCountDown, results, 0);
		try{
			taskCountDown.await();
		} catch (java.lang.InterruptedException e){
			
		}
		callbackContext.success(results.get(0));
	}
