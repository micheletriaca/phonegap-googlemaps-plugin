...
-(NSMutableDictionary *)createMarkerFromJsonArgument_:(NSDictionary *)json
{
    ...
    if (iconProperty) {
        ...
        [self setIcon_:marker iconProperty:iconProperty pluginResult:nil callbackId:nil];
    }
    return result;
...
{
    NSArray *multipleMarkerOptions = [command.arguments objectAtIndex:1];
    NSMutableArray *resultsArray = [[NSMutableArray alloc] initWithCapacity:[multipleMarkerOptions count]];
    for (NSDictionary* json in multipleMarkerOptions){
        NSMutableDictionary *result = [self createMarkerFromJsonArgument_:json];
        [resultsArray addObject:result];
    }
    CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsArray:resultsArray];
...
-(void)setIcon_:(GMSMarker *)marker iconProperty:(NSDictionary *)iconProperty
   pluginResult:(CDVPluginResult *)pluginResult
    ...
    if (iconPath) {
        ...
        if (range.location == NSNotFound) {
            ...
            if ([iconPath rangeOfString:@"data:image/"].location != NSNotFound &&
                ...
                NSString *hashKey = [NSString stringWithFormat:@"base64_%lu", (unsigned long)iconPath.hash];
                
                image = [self.iconCache objectForKey:hashKey];
                if (image == nil) {
                    /**
                     * Base64 icon
                     */
                    ...
                    [self.iconCache setObject:image
                                       forKey:hashKey];
                }
    ...
    if (pluginResult != nil && callbackId != nil){
